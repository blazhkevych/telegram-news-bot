...
LLM_PROVIDERS = [p for p in [
    ...
    {"name": "Cerebras",
     ...
     # ПЕРШИЙ за бойовим заміром, а не за теорією. Обидва прогони після
     # перебудови ланцюга (17.08, 10:00 і 10:02 UTC) Cerebras написав повністю:
     # «📈 Моделі: Cerebras×5» двічі поспіль, без жодної відмови. Він єдиний,
     # хто поєднує сильну модель зі швидкістю: gpt-oss-120b тут крутиться на
     # спеціалізованому залізі й вкладається в LLM_TIMEOUT, тоді як та сама
     # модель у NVIDIA стабільно не встигає (див. нижче).
     # Ліміт 1 млн токенів/добу — це НЕ покриває наш добовий потік цілком
     # (~680 викликів ≈ 1-2 млн токенів), тому решта ланцюга не декоративна:
     # коли Cerebras вичерпається, естафета йде далі. Контекст на безкоштовному
     # тарифі 8K — якщо курація почне впиратись, це перше місце, куди дивитись.
     # Запасні перевірені llm_check 17.08: gemma-4-31b жива; llama-3.3-70b
     # прибрано — 404, такої назви в каталозі немає.
    ...
    {"name": "NVIDIA",
     ...
     # ⚠️ gpt-oss-120b ТУТ НЕ ПРАЦЮЄ, хоч і є в каталозі — і це вартий уваги
     # урок. 17.08 його поставили сюди першим на тій підставі, що це та сама
     # модель, що в Groq. Наслідок: 10 з 10 викликів у двох прогонах поспіль
     # впали в таймаут, NVIDIA дала 0% замість колишніх 63%. Причина не в
     # квоті, а в класі моделі: gpt-oss-120b — reasoning-модель, і на спільному
     # залізі NVIDIA вона просто не встигає (llm_check того ж дня: основна —
     # таймаут, обидві запасні — «працює»). Наявність моделі в каталозі нічого
     # не каже про її ЛАТЕНТНІСТЬ у конкретного провайдера.
     # Тому першим — deepseek-v4-flash: швидкий, без «міркувань», саме він і
     # давав ті 63%. Ярлик з датою — не описка (БАГ-016): голе ім'я померло
     # 07.08.2026 з 410 «end of life». Каталог відкритий без ключа:
    ...
    {"name": "Groq",
     ...
     # сама, що в Cerebras/Cloudflare, тож втрати якості від цього немає — це
     # чисто розподіл дефіциту. Показово: llm_check б'є Groq п'ятитокенним
     # пробником і бачить 200 «працює», а бойові виклики того ж прогону ловлять
     # 429 — ліміт саме ТОКЕННИЙ, не запитний, тож дешевий пробник у нього не
     # впирається. llama-3.3-70b-versatile прибрано — 404, немає в каталозі.
    ...
    {"name": "Gemini",
     ...
     # Останній резерв. gemini-3.5-flash — актуальна GA (БАГ-008), і llm_check
     # 17.08 підтвердив живою саме її: питання «чи існує ще ця назва», яке
     # ревізія лишила відкритим (пошук давав суперечливі імена), закрите
     # бойовою перевіркою. `gemini-flash-latest` — псевдонім, який Google рухає
     # сам, тобто найкращий запасний. gemini-2.5-flash-lite прибрано: 404.
