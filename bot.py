...
def limit_hint(r):
    ...
    від неї відповідь за 0,5 с. Гіпотеза була «ліміт на хвилину» (лікується
    паузою). Перший же прогін із цим рядком (23.09 14:15 UTC) її СПРОСТУВАВ:
    «TPD: ліміт 200000, використано 199182, запит 2849, retry-after 878 с».
    Тобто добова квота з КОВЗНИМ вікном: за 15 хв між прогонами звільняється
    якраз на один виклик — курацію (~3 тис. токенів × ~100 прогонів ≈ 300 тис.
    на добу, більше за всю квоту 120b). Пауза тут не допоможе.
